...
# WAF / anti-bot / CDN interstitials - a blanket block the origin never served (it fires on the "suspicious"
# word, e.g. wp-config.php, while the random calibration probes sail through, so the block looks "distinct" and
# slips the soft-404 gate). Matched on SPECIFIC vendor block-page phrases - never a bare product name a real
# page might mention - so a genuine 401/403 for an actual resource is still reported.
_WAF_MARKERS = (
    ...
    "the requested url was rejected",                    # Imperva / F5 BIG-IP ASM
    "this website is using a security service to protect itself",
    "generated by cloudfront",                           # CloudFront block/error
)


def is_waf_block(status, body: str) -> bool:
    """A WAF / anti-bot / CDN block or challenge page (Cloudflare 'Attention Required' / 'Just a moment',
    Imperva/Incapsula, F5 ASM, CloudFront) - never a real discovered path, so it must not be reported nor
    recursed into. Keyed on specific block-page phrases, so it does not swallow a legitimate 403/401."""
    hay = (body or "")[:6000].lower()
...
def _dir_kind(word: str, sig: dict) -> str:
    """Classify a hit for recursion. 'redir' = definitive directory (it redirects to its OWN trailing-slash
    form). 'maybe' = extension-less final segment - dir-like, but `login`/`graphql`/`config`/`HEAD` are files or
    app routes, not directories, so this is only a CANDIDATE to confirm with a probe. '' = has an extension
    (foo.php, backup.sql) - a file, never recursed into."""
    seg = word.rstrip("/").split("/")[-1]
    if sig["loc"].endswith("/") and sig["loc"].rstrip("/").endswith(seg):
        return "redir"
    return "maybe" if "." not in seg else ""


def _confirm_dir(make_url, word: str, method: str, sess, deadline: float) -> bool:
    """Confirm an extension-less candidate is a REAL directory before recursing: its trailing-slash form must
    exist. A file (.git/config, backup.sql, .env) and most app endpoints answer 404 for `<path>/`; a directory
    serves an index (2xx) or redirects. One probe per candidate - candidates are the few discovered paths, not
    the whole wordlist - so this is cheap and stops path-bust descending into files/routes (where a WAF would
    otherwise manufacture a per-child 403 at every level)."""
    if time.time() >= deadline:
        return False
    r = http.send(method, make_url(word.rstrip("/") + "/"), sess=sess, timeout=8, allow_redirects=False)
    st = r["status"]
    if st is None or st == 404:
        return False
    return not is_waf_block(st, r["body"])
...
def scan(make_url, method: str, words: list, sess, deadline: float, codes: set, dbg) -> tuple:
    ...
    for i, word in enumerate(words, 1):
        ...
        if is_ghost(sig, baselines):
            continue
        if is_waf_block(st, r["body"]):           # a firewall/anti-bot block page, not a real path - never report
            dbg(f"  [WAF] {url} -> {st} firewall/challenge page, skipped")
            continue
        if st in codes:
            sk = _sk(st, sig)
            ...
            fsig.append(sk)
            kind = _dir_kind(word, sig)
            if kind:
                dir_cands.append((word, sk, kind))
        elif st in _REDIRECT:
            kind = _dir_kind(word, sig)
            if kind:
                dir_cands.append((word, _sk(st, sig), kind))   # a directory reachable only via a redirect

    # Recurse only into REAL directories: not part of a catch-all cluster, and - for extension-less candidates
    # (which could be files or app routes) - confirmed to expose a trailing-slash form. A redirect to the path's
    # own slash is already definitive. This keeps path-bust out of files (.git/config, backup.sql) and endpoints
    # (/login, /graphql), where a WAF would otherwise fabricate a per-child 403 at every level.
    dirs = []
    for (w, sk, kind) in dir_cands:
        if sk in catchall:
            continue
        if kind == "redir" or _confirm_dir(make_url, w, method, sess, deadline):
            dirs.append(w)
