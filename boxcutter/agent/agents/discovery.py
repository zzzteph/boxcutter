...
class Discovery(Agent):
    ...
    def objective(self, ctx):
        ...
        return (
            ...
            "3. `dirsearch <base>` AND `dirb <base>` for unlinked dirs/files - run BOTH, their wordlists DIFFER so "
            "each surfaces paths the other misses. MANDATORY recursion: the moment a hit reveals an "
            ...
            f"{wb}"
            "\nVERIFY every path is REAL before you keep it (a 200 proves NOTHING on a catch-all app): many "
            "apps route unknown paths to a catch-all that returns 200 + the same index page. FIRST request a "
            "random bogus path (e.g. /zzz<rand>) to fingerprint the catch-all (status + body size/text); then "
            "DISCARD any discovered or guessed path whose response MATCHES that baseline - it is NOT a real "
            "endpoint, do not report it, rank it, or hand it off. Only paths returning content DISTINCT from the "
            "catch-all are real; never invent path variants (/admin/x, /pic/x) and treat their catch-all 200 as a hit.\n"
