...
class Api(Agent):
    ...
    def objective(self, ctx):
        return (
            ...
            "injection. CRITICAL for POST/PUT: build the body FROM the swagger-parser schema - a real JSON object "
            "with EVERY required field filled with a valid-looking value and {FUZZ} in ONE field at a time (e.g. "
            "--data '{\"email\":\"{FUZZ}\",\"password\":\"Passw0rd!\"}' --header 'Content-Type: application/json'). "
            "NEVER `--data {FUZZ}` (one payload as the whole body just 400s and tests nothing).\n\n"
            ...
            "artifacts.endpoints (so lateral re-sweeps them with the token); for each POST/PUT, the request-body "
            "FIELD LIST from the schema in artifacts.notes (e.g. 'POST /Login body: email, password') so fuzzer/"
            "lateral can build a real body instead of garbage; findings with reproduce argv.\n"
