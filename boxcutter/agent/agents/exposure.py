...
class Exposure(Agent):
    ...
    def objective(self, ctx):
        return (
            ...
            "2. EXPOSED SOURCE/VCS: for the base and every admin/panel/internal path on the surface, probe "
            "`http-request <path>/.git/HEAD` (also try /.git/config, /.svn/, /.env, /config.php.bak, /backup.zip). "
            "If a .git is live, `git-extract <path>/` and `scan-secrets` the dumped files - source reveals new "
            "endpoints, secrets, and HOW auth/IDs/hashes are built. Report those as leads (cls=secret / rce-lead) "
            "and put new endpoints/creds in artifacts so lateral can chain them.\n"
            ...
            "4. OPEN PANELS: `screenshot` admin/login/management paths to spot a console that loads without auth.\n\n"
