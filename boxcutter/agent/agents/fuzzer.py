...
class Fuzzer(Agent):
    ...
    def objective(self, ctx):
        return (
            ...
            "Get the field names from `swagger-parser <spec>` or the api/prior handoff and fill EVERY required field "
            "with a valid value, {FUZZ} in one - NEVER `--data {FUZZ}` alone (one payload as the whole body just 400s)\n"
            ...
            "NoSQL / GraphQL / error-disclosure + time-blind. Escalate a confirmed SQLi with `sqlmap <url> "
            "--opt-args \"--banner --current-user --dbs --tables --batch\"` (then --dump a SMALL sample) to PROVE "
            "and QUANTIFY real data access - enumerate, don't stop at the error. For a precise re-check of one "
            "payload use `fuzz <url> --payload \"<p>\" --pattern \"REGEX\"`.\n\n"
