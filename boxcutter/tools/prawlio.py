...
def _deauthed(url: str, base_host: str) -> bool:
    ...
    partner-hub.* redirected to access.*), so the app's own pages are never mistaken for a login drop."""
    if not url:
        return False
...
def _reauth(base_url: str, sid: str, headers: list, grid, trace, login_flow: list, subs: dict) -> str:
...
def _explore(base_url: str, base_host: str, provider, sid: str, headers: list, grid, trace,
    ...
    for step in range(max(1, hard_cap)):
        ...
        if login_flow and reauths < max_reauth and _deauthed(last_url, base_host):
            ...
            last_url = _reauth(base_url, sid, headers, grid, trace, login_flow, subs)
        ...
        if text.strip():
            debug_print("prawlio[crawl]> " + text.strip()[:200])
        if not calls:
...
def add_arguments(parser) -> None:
    ...
    parser.add_argument("--context", default="", metavar="TEXT",
                        help="Free-text briefing; credentials mentioned here are parsed out and used (like logio)")
    ...
    parser.add_argument("--max-steps", dest="max_steps", type=int, default=14, help="Max login agent steps per stage")
    ...
    add_header_arg(parser)          # rides every login + crawl request (e.g. a Tester-Token)
    add_common_args(parser)


def _call(argv: list, headers: list) -> tuple:
    ...
    console; only stdout (the JSON envelope) is captured."""
...
def crawl(provider, base_url: str, subs: dict, grid=25, trace=None, stop_after: int = 50,
          crawl_steps: int = 60, headers=None, login_max_steps: int = 14) -> dict:
    ...
    res = logio_login(provider, base_url, subs, grid=grid, trace=trace, headers=headers, max_steps=login_max_steps)
    ...
    _call(vd_argv, headers)                               # replay the login into the persistent crawl session
    items = _explore(base_url, base_host, provider, _SESSION, headers, grid, trace,
                     stop_after, crawl_steps, login_flow, subs)
    seen: set = set()
    reqs: list = []
    for it in items:
...
def run(args) -> int:
    ...
    try:
        ...
        if args.base_url:
            logio_argv += ["--base-url", args.base_url]
        if args.creds:
        ...
        env, _raw = _call(logio_argv, headers)
        ...
        items = _explore(base_url, base_host, provider, _SESSION, headers, grid, trace,
                         args.stop_after, args.crawl_steps, login_flow, subs)
    finally:
