...
class Context:
    ...
    def record(self, argv, out):
        """Shared-runner compatibility (orca's Runner calls ctx.record(argv, out)): log the tool call and its
        response. Kept OUT of the decision trail - it's raw tool evidence, not an agent decision."""
    ...
    def _norm_target(self, target: str) -> str:
        return (str(target or "").strip().rstrip("/").lower()) or (self.base_url or "").rstrip("/").lower()

    def log_commission(self, executor: str, target: str, handoff: dict):
        key = f"{executor}|{self._norm_target(target)}"
        slot = self.commissions.setdefault(key, {"runs": 0, "findings": 0, "produced": 0})
        slot["runs"] += 1
        slot["findings"] += len(handoff.get("findings") or [])
        art = handoff.get("artifacts") or {}
        slot["produced"] += len(art.get("endpoints") or []) + len(art.get("tokens") or [])

    def was_committed(self, executor: str, target: str) -> bool:
        return f"{executor}|{self._norm_target(target)}" in self.commissions
    ...
    def is_dead_commission(self, executor: str, target: str) -> bool:
        ...
        slot = self.commissions.get(f"{executor}|{self._norm_target(target)}")
    ...
    def advice_outcomes(self, name: str, last: int = 6) -> str:
        """One profile's own advice history and what came of it - so it can argue from / build on it."""
        ...
        for sid in sug_ids[-last:]:
            ...
            out.append(f"  {sid} {s.summary}: {tag}" + (f" — {v.get('why')}" if v.get("why") else ""))
        return "\n".join(out)
    ...
    def add_identity(self, label, headers, source=""):
    ...
    def merge_handoff(self, handoff: dict, by: str, refs=None):
        ...
        art = handoff.get("artifacts") or {}
        self.add_urls(art.get("endpoints") or [])
        for i, tok in enumerate(art.get("tokens") or []):
            if isinstance(tok, dict) and tok.get("header"):
                self.add_identity(f"H{i + 1}", ["--header", tok["header"]], tok.get("source", by))
    ...
    def recent_trail(self, n: int = 12) -> str:
        if not self.trail:
